...
class QueryBuilder
{
    ...
    {
        if (is_array($column)) {
            return $this->parseArrayQuery($column, $boolean);
        }
    ...
    protected function parseArrayQuery(array $query, string $boolean = 'and'): self
    {
        foreach ($query as $key => $value) {
            if ($key === '$or' || $key === '$and') {
                $logic = $key === '$or' ? 'or' : 'and';
                $this->addNestedWhere($boolean, function ($q) use ($value, $logic) {
                    foreach ($value as $subQuery) {
                        $q->parseArrayQuery($subQuery, $logic);
                    }
                });
                continue;
            }
            ...
            if (is_string($key) && str_starts_with($key, '$')) {
                $op = $this->mapArrayOperator($key);
                if (is_array($value)) {
                    foreach ($value as $col => $val) {
                        $this->processArrayValue($col, $val, $op, $boolean);
                    }
                }
                continue;
            }

            $this->processArrayValue($key, $value, '=', $boolean);
        }
        return $this;
    }

    protected function processArrayValue($column, $value, $defaultOperator, $boolean)
    {
        if (is_array($value)) {
            $hasOp = false;
            foreach ($value as $k => $v) {
                if (is_string($k) && str_starts_with($k, '$')) {
                    $op = $this->mapArrayOperator($k);
                    if ($k === '$in') {
                        $this->whereIn($column, (array)$v, $boolean);
                    } elseif ($k === '$nin') {
                        $this->whereNotIn($column, (array)$v, $boolean);
                    } else {
                        $this->addWhere($boolean, $column, $op, $v);
                    }
                    $hasOp = true;
                }
            }
            ...
            $this->addWhere($boolean, $column, $defaultOperator, $value);
        }
    }

    protected function mapArrayOperator(string $arrayOp): string
    {
        return match ($arrayOp) {
            '$eq' => '=',
            '$ne' => '!=',
            '$gt' => '>',
            '$gte' => '>=',
    ...
    public function orderBy(string $column, string $direction = 'ASC', $once = false): self
    {
        if ($once) {
            $this->orders = [];
    ...
    public function debugSql($encode=false): string
    {
        $sql = $this->toSql();
