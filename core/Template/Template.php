...
class Template
{
    ...
    public function render(string $view, array $data = []): string
    {
        ...
        if ($this->layout) {
            ...
            $content = preg_replace('/<\/head>/i', '<script>window.languageData = ' . \Pluto\Lang::getInstance() . ';window.__=(e,t={})=>{let n=window.languageData&&window.languageData[e]?window.languageData[e]:e;for(const e in t)if(Object.hasOwnProperty.call(t,e)){const r=t[e];n=n.replace(new RegExp(":"+e,"g"),r)}return n}</script></head>', $content);
            if (getenv('USE_PLUTO_JS') === 'true') {
                $plutoCssLink = '<link id="pluto-css" rel="stylesheet" href="' . getenv('HOST') . '/core/styles">';
                $plutoJsScript = '<script src="' . getenv('HOST') . '/core/scripts"></script>';
    ...
    protected function compile(string $content): string
    {
        ...
        $content = preg_replace('/@cookie\s*\(\s*\'(.*?)\'\s*\)/', '<?php echo request()->cookie(\'$1\'); ?>', $content);
    ...
    public static function timeFormat($date, $format = 'H:i:s')
    {
        $date = (!\is_numeric($date)) ? \strtotime($date) : $date;
        return \date($format, $date);
    }
    ...
    public static function secondsToHHMMSS($second)
    {
