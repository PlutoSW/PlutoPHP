...
class Application
{
    ...
    private function systemRoute()
    {
        ...
        foreach ($routes as $route) {
            $this->router->addRoute($route[0], $route[1], $route[2], ['skip' => true]);
        }
    }
    ...
    public static function uiLog($message): void
    {
        $logDir = BASE_PATH . '/storage/logs';
        if (!is_dir($logDir)) {
            mkdir($logDir, 0777, true);
    ...
    private function handleException(\Throwable $e): Response|String
    {
        ...
        if ($isDebug) {
            if ($this->request->isJson()) {
                return $this->response->json(['success' => false, 'message' => $e->getMessage() . " on " . $e->getFile() . ":" . $e->getLine(), 'error' => $e->getTrace()], 500);
            }
            ...
            if ($this->request->getHeader('Content-Type') === "application/json") {
                return $this->response->json(['success' => false, 'error' => $message], 500);
            }
