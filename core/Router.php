...
class Router
{
            ...
            foreach ($routes as $path => $route) {
                ...
                if ($perm) {
                    if (\is_array($perm)) {
                        ...
                        $permissions[$perm] = $permissions[$perm] + 1;
                    }
                }
            }
        }
    ...
    public function registerRoutesFromController(string $controllerClass)
    {
        ...
        foreach ($reflectionClass->getMethods() as $method) {
            ...
            foreach ($attributes as $attribute) {
                ...
                $this->addRoute($route->method, $route->path, [$controllerClass, $method->getName()], $route->middleware, $route->permission);
            }
        }
    }

    public function addRoute(string $method, string $path, $callback, array $middleware = [], string|array $permission = "")
    {
        $this->routes[\strtoupper($method)][$path]['callback'] = $callback;
        $this->routes[\strtoupper($method)][$path]['middleware'] = $middleware;
        $this->routes[\strtoupper($method)][$path]['permission'] = $permission;
    }
    ...
    public function dispatch()
    {
            ...
            foreach ($this->routes[$method] ?? [] as $routePath => $cb) {
                if (strpos($routePath, '{') !== false) {
                    ...
                    if (preg_match("#^$pattern$#", $path, $matches)) {
                        ...
                        $permission = $cb['permission'] ?? "";
                        return $this->executeCallback($callback, $params, $middleware, $permission);
                    }
                }
            }
        ...
        return $this->executeCallback($callback, [], $middleware, $permission);
    }

    private function executeCallback($callback, array $params = [], array $middleware = [], string|array $permission = "")
    {
        $this->request->permission = $permission;

        $finalRequest = function () use ($callback, $params) {
        ...
        $middlewaresToRun = [];
        if ($this->globalMiddleware && class_exists($this->globalMiddleware) && !isset($middleware["skip"])) {
            $middlewaresToRun[] = $this->globalMiddleware;
        }

        if (count($middleware) && !isset($middleware["skip"])) {
