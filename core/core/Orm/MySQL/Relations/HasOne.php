...
class HasOne extends Relation
{
    public function getRelated(Model $parent)
    {
        ...
        return $this->relatedClass::where($this->foreignKey, '=', $val)->first();
    }
    ...
    {
        $map = [];
        foreach ($related as $r) $map[$r->{$this->foreignKey}] = $r;
        foreach ($parents as $p) {
            $p->{$this->relatedPropertyName()} = $map[$p->{$this->localKey}] ?? null;
            // also set attribute accessible
            $p->attributes[$this->relatedPropertyName()] = $map[$p->{$this->localKey}] ?? null;
        }
    }
